...
var stopwatch_default = Node2.create({
  name: "stopwatch",
  group: "block",
  atom: true,
  addAttributes() {
  ...
  renderHTML({ HTMLAttributes }) {
    return ["stopwatch-view", mergeAttributes(HTMLAttributes)];
  },
  addNodeView() {
    ...
    `;
      function change_background() {
        let background_index = node.attrs.background;
      ...
      let timer;
      let isRunning = node.attrs.playing;
      let startTime;
      let elapsedTime = 0;
      function updateTime() {
      ...
      dom.querySelector("#startStopBtn").addEventListener("click", () => {
        if (typeof getPos === "function") {
          startStop();
        }
      });
      dom.querySelector("#resetBtn").addEventListener("click", () => {
        if (typeof getPos === "function") {
          reset();
        }
      });
      dom.querySelector("#change_bg_btn").addEventListener("click", () => {
        if (typeof getPos === "function") {
          change_background();
        }
      });
