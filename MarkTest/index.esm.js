...
// index.js
var curlyRegex = /\{([^}]+)\}$/;
var globalCurlyRegex = /\{[^}]+\}/g;
var UtilityTest_default = Node2.create({
  name: "replaceCurlyContent",
  // Input rule: when typing finishes with `...}` this will replace the whole `{...}` with "hello"
  addInputRules() {
    return [
      new InputRule(curlyRegex, (state, match, start, end) => {
        const { tr } = state;
        tr.insertText("hello", start, end);
        return tr;
      })
    ];
  },
  // Handle paste: if pasted text contains any `{...}` segments,
  // replace each segment with "hello" and insert the modified text.
  addProseMirrorPlugins() {
    return [
      new this.editor.view.Plugin({
        props: {
          handlePaste: (view, event, slice) => {
            ...
            if (!globalCurlyRegex.test(text)) return false;
            event.preventDefault();
            const replaced = text.replace(globalCurlyRegex, "hello");
            const { state, dispatch } = view;
            const node = state.schema.text(replaced);
...
export {
  UtilityTest_default as default
};
